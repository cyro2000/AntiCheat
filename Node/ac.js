...
const RUN_DIR = path.dirname(process.execPath && process.execPath.endsWith('node.exe') ? process.argv[1] : process.execPath);
...
const W = '\x1b[0m', WH = '\x1b[97m', GR = '\x1b[90m', RD = '\x1b[91m';
const GN = '\x1b[92m', YL = '\x1b[93m', CY = '\x1b[96m', MG = '\x1b[95m';
...
let scanCounters = {};
let allFindings = {};
let habibiStatus = 'idle';

function setHabibiStatus(s) { habibiStatus = s; }
function getHabibiStatus() { return habibiStatus; }
function habWidget() {
  const s = getHabibiStatus();
  if (s === 'running') return c(GN)('●') + ' ' + c(GN)('Habibi');
  if (s === 'warn') return c(YL)('●') + ' ' + c(YL)('Habibi');
  if (s === 'crashed') return c(RD)('●') + ' ' + c(RD)('Habibi');
  return c(GR)('●') + ' ' + c(GR)('Habibi');
}
...
function statusBar() {
  ...
  return `  ${c(GN)('●')} ${c(WH)(g)}  ${c(YL)('●')} ${c(WH)(y)}  ${c(RD,BOLD)('●')} ${c(WH)(r)}  ${c(GR)('files:')} ${c(WH)(t)}  ${c(GR)('jars:')} ${c(WH)(j)}  │  ${habWidget()}`;
}
...
function drawProgress(label, done, total, eta = '') {
  ...
  const bar = c(CY)('█'.repeat(filled)) + c(GR)('░'.repeat(bw - filled));
  const etaS = eta ? c(GR)(`  ETA ${eta}`) : '';
  process.stdout.write(`\r\x1b[K  ${c(GR)(label.padEnd(18))} ${bar}  ${c(WH)(`${done}/${total}`)}${etaS}  │ ${habWidget()}   `);
}
...
function section(title) {
  const tag = c(GR)('[') + c(CY)('>') + c(GR)('] ') + c(WH, BOLD)(title);
  const pad = Math.max(2, 76 - title.length - 16);
  p('');
  p(tag + ' '.repeat(pad) + habWidget());
}
...
function row(label, val, st, w = 36) {
  ...
  else if (st === 'info') vb = c(CY)(`  ${val}`);
  else vb = c(GR)(`  ${val}`);
  p(lb + vb);
}

const ASCII_LOGO = [
  '   ___   _  ___________________ _________ ______',
  '  / _ | / |/ /_  __/  _/ ___/ // / __/ _ /_  __/',
  ' / __ |/    / / / _/ // /__/ _  / _// __ |/ /',
  '/_/ |_/_/|_/ /_/ /___/\\___/_//_/___/_/ |_/_/',
];
const LOGO_GRADIENT = [232, 240, 248, 255];
...
function printLogo() {
  ...
  ASCII_LOGO.forEach((line, i) => {
    ...
    p(indent + '\x1b[3m' + `\x1b[38;5;${shade}m` + line + W + '\x1b[0m');
  });
...
function banner() {
  clear(); p('');
  printLogo(); p('');
  p(c(GR)('[') + c(CY)('>') + c(GR)('] ') + c(WH)('AntiCheat ready.') + c(GR)('  /help  ·  Tab = autocomplete  ·  Ctrl+C = cancel scan'));
  if (!isAdmin()) p(c(GR)('[') + c(YL)('!') + c(GR)('] ') + c(YL)('Not Administrator — registry + prefetch limited'));
  p(c(GR)('[') + c(CY)('i') + c(GR)('] ') + c(WH)('Preset: ') + c(CY)(activePreset.name) + c(GR)('   ') + habWidget());
  p('');
}
...
function getHabibiConfig() { return loadedData.habibi || DEFAULT_DATA.habibi; }
function getKnownLegitModIds() { return new Set(getHabibiConfig().known_legit_mod_ids || []); }
...
function readZipEntries(jarPath) {
  ...
  while (cdPos < cdOffset + cdSize) {
    ...
    const flags = buf.readUInt16LE(cdPos + 8);
...
function readZipEntry(jarPath, buf, entry) {
  ...
  const { localOffset, compression, compSize, uncompSize } = entry;
...
function analyzeJar(jarPath, customStrings, paranoid = false) {
  ...
  const hcfg = getHabibiConfig();
  const moduleNames = getAllModuleNames();
  const GENERIC_SEGS = new Set(['me','com','net','org','io','client','impl','mod','internal','core','dev']);

  let buf;
  try { buf = fs.readFileSync(jarPath); } catch (e) { r.verdict = 'ERROR'; r.skipReason = String(e).slice(0, 80); return r; }

  let entries;
  try { entries = readZipEntries(jarPath); } catch (e) { r.verdict = 'ERROR'; r.skipReason = 'Bad ZIP: ' + String(e).slice(0, 60); return r; }
...
function formatHabibiResult(r) {
  const lines = [];
  if (r.skipReason && r.verdict === 'CLEAN') { lines.push(c(GR)(`  ├─ ${r.filename.padEnd(48)}`) + c(GN)('  ✓ legit')); return lines; }
  if (r.verdict === 'CLEAN') { lines.push(c(GR)(`  ├─ ${r.filename.padEnd(48)}`) + c(GN)('  ✓ clean')); return lines; }
  if (r.verdict === 'ERROR') { lines.push(c(GR)(`  ├─ ${r.filename.padEnd(48)}`) + c(GR)(`  — ${r.skipReason}`)); return lines; }
  ...
  if (r.mixinPkgs.length) lines.push(c(GR)('  │  ') + c(WH)('Mixins   ') + c(CY)(r.mixinPkgs.slice(0,3).join(', ')));
  ...
  if (r.cheatStrings.length) {
    ...
    lines.push(c(GR)('  │  ') + c(WH)('Cheats   ') + color(cs));
  }
        ...
        fs.readdirSync(crashDir).slice(-10).forEach(fname => {
          ...
          try {
            ...
            for (const [clientName, terms] of Object.entries(logTermsMap)) {
              for (const term of terms) {
                if (content.includes(term.toLowerCase())) {
                  tlog(`CRASH REPORT HIT  [${clientName}]  ${fname}`, 'bad');
...
function phase1ConfigScan(gameRoot, maxDepth = 4) {
...
function phase2Habibi(modsDir, paranoid = false) {
  section('PHASE 2 — Habibi Mod Analyzer');
  divider();
  if (!fs.existsSync(modsDir)) { tlog(`Mods directory not found: ${modsDir}`, 'info'); return; }
  let jars;
  try { jars = fs.readdirSync(modsDir).filter(f => f.toLowerCase().endsWith('.jar')); } catch { return; }
  if (!jars.length) { tlog('No JARs in mods directory', 'info'); return; }
  tlog(`Habibi scanning ${jars.length} JARs${paranoid ? ' (paranoid)' : ''}`, 'scan');
  setHabibiStatus('running');
  const custom = ghostState.active ? ghostState.strings : null;
  const start = Date.now();
  for (let i = 0; i < jars.length; i++) {
    drawProgress(`Habibi [${i}/${jars.length}]`, i, jars.length, etaStr(Date.now() - start, i + 1, jars.length));
    ...
    try {
      ...
      if (r.verdict === 'CHEAT') {
        ...
        tlog(`HABIBI  CHEAT  ${c(RD, BOLD)(name)}  ·  ${r.filename}  ·  ${c(YL)(r.probability() + '%')}`, 'bad');
        allFindings.habibi.push({ path: jarPath, verdict: 'CHEAT', client: name, confidence: r.probability(), strings: r.cheatStrings.slice(0,10) });
        allFindings.summary.private_hits++;
        formatHabibiResult(r).forEach(l => p(l));
      } else if (r.verdict === 'SUSPICIOUS') {
        scanCounters.yellow++;
        tlog(`HABIBI  SUSPICIOUS  ${r.filename}`, 'warn');
        allFindings.habibi.push({ path: jarPath, verdict: 'SUSPICIOUS', strings: r.cheatStrings.slice(0,5) });
      } else { scanCounters.green++; }
    } catch (e) { tlog(`Habibi error on ${jars[i]}: ${e}`, 'warn'); setHabibiStatus('warn'); }
  }
  process.stdout.write('\r\x1b[K');
  const cheats = scanCounters.red;
  if (cheats > 0) { setHabibiStatus('warn'); tlog(`Habibi done · ${jars.length} JARs · ${c(RD,BOLD)(cheats + ' CHEAT(S)')} · ${((Date.now()-start)/1000).toFixed(1)}s`, 'bad'); }
  else { setHabibiStatus('idle'); tlog(`Habibi done · ${jars.length} JARs · clean · ${((Date.now()-start)/1000).toFixed(1)}s`, 'ok'); }
}
...
function buildCaseFiles() {
  ...
  for (const e of allFindings.habibi) {
...
function saveReport() {
  try {
    ...
    const reportPath = path.join(os.homedir(), 'Desktop', `ac_report_${ts}.json`);
    fs.writeFileSync(reportPath, JSON.stringify({ timestamp: ts, preset: activePreset.name, scan_counters: scanCounters, findings: allFindings }, null, 2));
    return reportPath;
  } catch { return null; }
}
...
function printVerdict() {
  ...
  if (r === 0 && y === 0) p(c(GN, BOLD)('  ✓  CLEAN — no cheat indicators found'));
    ...
    const names = [...new Set([...allFindings.habibi, ...allFindings.config_hits].map(e=>e.client).filter(Boolean))];
    p(c(RD, BOLD)(`  ✗  FLAGGED — ${r} indicator(s) · ${names.slice(0,4).join(', ')}`));
  }
  divider();
  const rp = saveReport();
  if (rp) p(c(DIM, CY)(`  Report → ${rp}`));
  p('');
}

function doFlash(arg = '') {
  SCAN_RUNNING = true; resetRefs();
  const target = arg.trim().replace(/"/g,'');
  const settings = activePreset.data || {};
  section('FLASH SCAN — config → logs → habibi → mods');
  divider();
  ...
  phase1ConfigScan(mc, settings.max_config_depth || 4);
  phase2Habibi(path.join(mc, 'mods'), settings.paranoid_mode || false);
  doDnsScan();
  doProcessScan();
  p(''); drawStatus(); p('');
  printVerdict();
...
function doPro(arg = '') {
  ...
  for (const root of roots) {
    ...
    phase1ConfigScan(root, settings.max_config_depth || 6);
    const mods = path.join(root, 'mods');
    if (fs.existsSync(mods)) phase2Habibi(mods, true);
  }
  if (!target) {
    const allJars = [];
    ...
    for (const drive of 'CDEFGH'.split('').map(d => `${d}:\\`)) {
      ...
      walkDir(drive, DRIVE_WALK_SKIP, (full, fname) => {
        ...
        if (Date.now() - hb > 1500) { process.stdout.write('\r\x1b[K  ' + c(GR)(`JAR hunt...  ${allJars.length} found so far`)); hb = Date.now(); }
      });
    }
    process.stdout.write('\r\x1b[K');
    ...
    if (allJars.length) {
      ...
      for (let i = 0; i < allJars.length; i++) {
        ...
        try {
          ...
          if (r.verdict === 'CHEAT') {
            ...
            tlog(`HABIBI  CHEAT  ${nm}  ·  ${allJars[i].slice(-60)}`, 'bad');
            allFindings.habibi.push({ path: allJars[i], verdict: 'CHEAT', client: nm, confidence: r.probability() });
            allFindings.summary.private_hits++;
      ...
      process.stdout.write('\r\x1b[K');
      setHabibiStatus(scanCounters.red > 0 ? 'warn' : 'idle');
    }
  }
  doRegistryScan();
  doPrefetchScan();
...
function doMax(arg = '') {
  ...
  if (!target) {
    ...
    let hashHits = 0;
    for (const jp of allJars) { const h = sha256File(jp); if (h && h in bad) { tlog(`HASH HIT  ${bad[h].client}  ·  ${path.basename(jp)}`, 'bad'); scanCounters.red++; hashHits++; } }
  ...
  for (const d of 'CDEFGH'.split('').map(d => `${d}:\\$Recycle.Bin`)) {
    ...
    walkDir(d, new Set(), (full, fname) => {
      if (/\.(jar|json)$/i.test(fname)) { tlog(`  Deleted file: ${full}`, 'warn'); allFindings.config_hits.push({ type:'recycle_bin', path: full }); scanCounters.yellow++; }
    });
  }
    ...
    try {
      fs.readdirSync(ud).forEach(fname => {
        ...
        for (const kw of looseKw) {
          if (kw && stem.includes(kw)) { tlog(`  FOUND  '${fname}' in ${path.basename(ud)}`, 'bad'); allFindings.config_hits.push({ type:'user_dir_tool', path: fpath, match: kw }); scanCounters.red++; break; }
        }
      });
    } catch {}
  }
  ...
  for (const sd of startupDirs) {
    ...
    try { fs.readdirSync(sd).forEach(fname => { const fl = fname.toLowerCase().replace(/[-_]/g,''); for (const kw of looseKw) { if (kw && fl.includes(kw)) { tlog(`  STARTUP  ${fname}`, 'bad'); scanCounters.red++; } } }); } catch {}
  }
  ...
  try {
    ...
    for (const line of hosts.split('\n')) {
      ...
      for (const domain of CHEAT_DOMAINS) { if (line.toLowerCase().includes(domain)) { tlog(`  HOSTS  ${line.trim().slice(0,70)}`, 'warn'); scanCounters.yellow++; hostHits++; } }
    }
    ...
    try {
      fs.readdirSync(modsDir).filter(f=>f.toLowerCase().endsWith('.jar')).forEach(fname => {
        ...
        try {
          ...
          const freq = {}; for (const b of data) freq[b] = (freq[b]||0)+1;
          const entropy = -Object.values(freq).reduce((s,c2)=>{ const p2=c2/data.length; return s+p2*Math.log2(p2); },0);
  ...
  try {
    ...
    const reports = fs.readdirSync(desktop).filter(f=>f.startsWith('ac_report_')&&f.endsWith('.json')).sort();
    if (reports.length > 1) {
      const prev = JSON.parse(fs.readFileSync(path.join(desktop, reports[reports.length-2]),'utf8'));
      const prevPaths = new Set((prev.findings?.habibi||[]).filter(h=>h.verdict==='CHEAT').map(h=>h.path));
      const curPaths = new Set(allFindings.habibi.filter(h=>h.verdict==='CHEAT').map(h=>h.path));
    ...
    for (let i = 0; i < deepJars.length; i++) {
      ...
      try {
        ...
        if (r.verdict === 'CHEAT') {
          ...
          allFindings.habibi.push({ path: deepJars[i], verdict:'CHEAT', client: nm, confidence: r.probability() });
        } else if (r.verdict === 'SUSPICIOUS') { scanCounters.yellow++; }
        else { scanCounters.green++; }
      } catch {}
    }
...
function cmdInspect(arg) {
  ...
  p(c(CY)(`  Habibi deep scan: ${path.basename(jarPath)}`)); p('');
  const r = analyzeJar(jarPath, null, true);
  formatHabibiResult(r).forEach(l => p(l));
  if (r.verdict === 'CHEAT' || r.verdict === 'SUSPICIOUS') {
    p(c(GR)('  Package tree:'));
    Object.entries(r.pkgTree).sort((a,b)=>b[1]-a[1]).slice(0,8).forEach(([pkg,cnt]) => p(c(GR)(`    ${pkg.padEnd(48)} ${cnt} classes`)));
  }
  p('');
}
...
function cmdPath(args) {
  ...
  if (sub === 'list') {
    p(c(CY, BOLD)('  Focus paths:')); (ghostState.focusPaths.length ? ghostState.focusPaths : ['(none)']).forEach(x => p(c(WH)(`    · ${x}`)));
    p(c(CY, BOLD)('  Strings:')); (ghostState.strings.length ? ghostState.strings : ['(none)']).forEach(x => p(c(WH)(`    · ${x}`)));
    p(''); return;
  }
...
function cmdGhost() {
  ...
  else { ghostState.active = true; p(c(GN)('  Ghost mode ON')); ghostState.paths.forEach(pp => p(c(GR)(`    path: ${pp}`))); ghostState.strings.forEach(s => p(c(GR)(`    string: ${s}`))); }
  p('');
}
...
function cmdStats() {
  ...
  const desktop = path.join(home, 'Desktop');
  let reports = [];
  try { reports = fs.readdirSync(desktop).filter(f=>f.startsWith('ac_report_')&&f.endsWith('.json')).reverse().slice(0,10); } catch {}
  ...
  for (const fname of reports) {
    try {
      ...
      const ts = fname.replace('ac_report_','').replace('.json','');
...
function cmdReport() {
  ...
  const desktop = path.join(home, 'Desktop');
  let reports = [];
  try { reports = fs.readdirSync(desktop).filter(f=>f.startsWith('ac_report_')&&f.endsWith('.json')).reverse(); } catch {}
...
function cmdWindow() {
  ...
  try {
    ...
    p(c(GN)('  ✓ New AntiCheat window opened'));
...
function cmdHelp() {
  ...
  const cmds = [
    ['/flash [path]',        'Config→Logs→Habibi→DNS→Process (.minecraft only)'],
    ...
    ['/path <path>',         'Permanently focus /flash and /pro on this path'],
    ...
    ['/window',              'Open second AntiCheat window'],
    ['/clear',               'Clear screen'],
    ['/exit',                'Exit'],
  ];
  const mw = Math.max(...cmds.map(([cmd])=>cmd.length)) + 2;
  cmds.forEach(([cmd, desc]) => p(c(CY,BOLD)(`  ${cmd.padEnd(mw)}`) + c(GR)('  ·  ') + c(WH)(desc)));
  p('');
}
...
async function repl() {
  ...
  while (true) {
    ...
    const prompt = c(GR)('  ') + gt + c(WH,BOLD)('>') + c(GR)(' ');
    let raw;
    ...
    try {
      switch(command.toLowerCase()) {
        ...
        case '/exit': case '/quit': p(c(GR)('  Goodbye.')); p(''); process.exit(0); break;
        case '/flash': doFlash(arg); break;
        case '/pro': doPro(arg); break;
        case '/max': doMax(arg); break;
