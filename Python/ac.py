...
def open_ref(n):
    ...
    try:
        if ref["is_folder"]:
            if os.name == "nt" and os.path.isfile(ref["path"]):
...
def set_habibi_status(s):
    global _HABIBI_STATUS
    _HABIBI_STATUS = s

def get_habibi_status():
    return _HABIBI_STATUS

def hab_widget():
    s = get_habibi_status()
    if   s == "running": return c(GN)("●") + " " + c(GN)("Habibi")
    elif s == "warn":    return c(YL)("●") + " " + c(YL)("Habibi")
    elif s == "crashed": return c(RD)("●") + " " + c(RD)("Habibi")
...
def section(title):
    tag = c(GR)("[") + c(CY)(">") + c(GR)("] ") + c(WH,BOLD)(title)
    visible_len = len(title) + 4
    pad = max(2, 76 - visible_len - 12)
    p()
    p(tag + " " * pad + hab_widget())
...
def banner():
    ...
    print_logo()
    p()
    p(c(GR)("[") + c(CY)(">") + c(GR)("] ") + c(WH)("AntiCheat ready.") + c(GR)("  Stripper Algorithm  ·  /help  ·  Tab = autocomplete"))
    if not is_admin():
        p(c(GR)("[") + c(YL)("!") + c(GR)("] ") + c(YL)("Not Administrator — registry + prefetch scans limited"))
    if not PSUTIL_OK:
        p(c(GR)("[") + c(YL)("!") + c(GR)("] ") + c(YL)("pip install psutil  — process scanning limited"))
    p(c(GR)("[") + c(CY)("i") + c(GR)("] ") + c(WH)("Preset: ") + c(CY)(active_preset["name"]) +
      c(GR)("   Data: ") + c(GR)(str(DATA_DIR)) + c(GR)("   ") + hab_widget())
    p()
...
PRESET_SKELETON_README = """ANTICHEAT — Preset Folder
...
Files in this preset:
  ...
  habibi.json            - scan tuning (thresholds, limits)
...
--------------------------------------------------------------------
habibi.json format (scan tuning):
--------------------------------------------------------------------
{
  "scan_class_limit_standard": 150,
  "scan_class_limit_paranoid": 999999,
...
PRESET_DEFAULTS = {
    "default": {
        ...
        "run_log_scan": True, "run_config_scan": True, "run_habibi_scan": True,
...
def HABIBI_CONFIG():
    return _loaded_data.get("habibi", DEFAULT_DATA["habibi"])

def KNOWN_LEGIT_MOD_IDS():
    return set(HABIBI_CONFIG().get("known_legit_mod_ids", []))
...
def format_habibi_result(r):
    lines = []
    if r.skip_reason and r.verdict == "CLEAN":
        lines.append(c(GR)(f"  ├─ {r.filename:<48}") + c(GN)("  ✓ legit"))
        return lines
    if r.verdict == "CLEAN":
...
def get_scan_roots():
    if ghost_state["active"] and ghost_state["paths"]:
        ...
        for name, paths in KNOWN_LAUNCHERS.items():
            for p2 in paths:
                if os.path.isdir(p2): roots.append(p2); break
...
def phase2_habibi(mods_dir, paranoid=False):
    ...
    if not jars:
        ...
        return
    tlog(f"Habibi scanning {len(jars)} JARs {'(paranoid)' if paranoid else ''}", "scan")
    set_habibi_status("running")
    custom = ghost_state["strings"] if ghost_state["active"] else None
    start = time.time()
    for i, fname in enumerate(jars):
        draw_progress(f"Habibi [{i}/{len(jars)}]", i, len(jars), eta_str(time.time()-start, i+1, len(jars)))
        ...
        try:
            ...
            if r.verdict == "CHEAT":
                ...
                tlog(f"HABIBI  CHEAT FOUND  {c(RD,BOLD)(name)}  ·  {r.filename}  ·  {c(YL)(str(prob)+'%')}", "bad")
                all_findings["habibi"].append({
                ...
                all_findings["summary"]["private_hits"] += 1
                for line in format_habibi_result(r): p(line)
            elif r.verdict == "SUSPICIOUS":
                scan_counters["yellow"] += 1
                tlog(f"HABIBI  SUSPICIOUS  {r.filename}", "warn")
                all_findings["habibi"].append({"path":jar_path,"verdict":"SUSPICIOUS","strings":r.cheat_strings[:5]})
            else:
                scan_counters["green"] += 1
        except Exception as e:
            tlog(f"Habibi error on {fname}: {e}", "warn")
            set_habibi_status("warn")
    sys.stdout.write("\r\033[K")
    elapsed = time.time() - start
    cheats = scan_counters["red"]
    if cheats > 0:
        set_habibi_status("warn")
        tlog(f"Habibi done · {len(jars)} JARs · {c(RD,BOLD)(str(cheats)+' CHEAT(S)')} · {elapsed:.1f}s", "bad")
    else:
        set_habibi_status("idle")
        tlog(f"Habibi done · {len(jars)} JARs · clean · {elapsed:.1f}s", "ok")
...
def live_instance_correlation(proc):
    ...
    if os.path.isdir(mods_dir):
        phase2_habibi(mods_dir, settings.get("paranoid_mode", False))
...
def build_case_files():
    ...
    for entry in all_findings.get("habibi", []):
...
def print_verdict():
    ...
    row("Habibi status", get_habibi_status(),
        "warn" if get_habibi_status()=="warn" else "clean" if get_habibi_status()=="idle" else "info")
    divider()
    if r == 0 and y == 0:
        ...
        names = list({
            ...
            for e in (all_findings.get("habibi",[]) + all_findings.get("config_hits",[]))
    ...
    try:
        ...
        report_path = os.path.join(os.path.expanduser("~"),"Desktop", f"ac_report_{ts}.json")
...
def do_flash(one_shot_arg=""):
    ...
    if settings.get("run_habibi_scan", True): phase2_habibi(os.path.join(mc,"mods"), settings.get("paranoid_mode", False))
...
def diff_against_last_report():
    ...
    try:
        reports = sorted([f for f in os.listdir(desktop) if f.startswith("ac_report_") and f.endswith(".json")])
    except Exception:
        reports = []
    if not reports:
    ...
    try:
        ...
        prev_paths = {h.get("path") for h in prev.get("habibi",[]) if h.get("verdict")=="CHEAT"}
        current_paths = {h.get("path") for h in all_findings.get("habibi",[]) if h.get("verdict")=="CHEAT"}
...
def cross_correlate_instances():
    ...
    confirmed = [h for h in all_findings.get("habibi",[]) if h.get("verdict")=="CHEAT"]
    if not confirmed:
...
def do_max(one_shot_arg=""):
    ...
    for root in roots:
        ...
        if os.path.isdir(mods):
            phase2_habibi(mods, paranoid=True)
    ...
    if not target:
        ...
        if all_jars:
            tlog(f"System-wide Habibi scan: {len(all_jars)} JARs","scan")
            set_habibi_status("running")
            start = time.time()
            for i,jar_path in enumerate(all_jars):
                ...
                try:
                    ...
                    if r.verdict=="CHEAT":
                        ...
                        if r.sha256_val: record_bad_hash(r.sha256_val, name, jar_path)
                        learn_structure(name, r.class_basenames)
                        tlog(f"HABIBI  CHEAT  {name}  ·  {jar_path[-60:]}","bad")
                        all_findings["habibi"].append({"path":jar_path,"verdict":"CHEAT","client":name,"confidence":r.probability(),"strings":r.cheat_strings[:8]})
                        all_findings["summary"]["private_hits"] += 1
                        for line in format_habibi_result(r): p(line)
                    elif r.verdict=="SUSPICIOUS":
                        scan_counters["yellow"] += 1
            ...
            sys.stdout.write("\r\033[K")
            set_habibi_status("warn" if scan_counters["red"]>0 else "idle")
...
def cmd_inspect(arg):
    ...
    p(c(CY)(f"  Habibi deep scan: {os.path.basename(path)}"))
    p()
...
def cmd_elevate():
    p()
    if is_admin():
        p(c(GN)("  ✓ Already running as Administrator")); p(); return
    try:
        exe = sys.executable
        ...
        if rc > 32:
            p(c(GN)("  ✓ Relaunching elevated — this window can be closed"))
        else:
            p(c(YL)("  UAC prompt was cancelled or failed"))
    except Exception as e:
        p(c(RD)(f"  ✗  {e}"))
    p()
...
def cmd_stats():
    ...
    try:
        reports = sorted(
            [f for f in os.listdir(desktop) if f.startswith("ac_report_") and f.endswith(".json")],
            reverse=True)[:10]
    except Exception:
        reports = []
    if not reports:
    ...
    for fname in reports:
        try:
            ...
            ts = fname.replace("ac_report_", "").replace(".json", "")
...
def cmd_find(query=None):
    ...
    if raw.strip().isdigit():
        ...
        if 0<=idx<len(subdir_list):
            ...
            if exists: os.startfile(open_path); p(c(GN)(f"  Opened: {open_path}"))
...
def cmd_report():
    ...
    try:
        reports = sorted(
            [os.path.join(desktop,f) for f in os.listdir(desktop) if f.startswith("ac_report_") and f.endswith(".json")],
            reverse=True)
    ...
    if raw!="__CTRLC__" and raw.strip().isdigit():
        idx=int(raw.strip())-1
        if 0<=idx<len(reports): os.startfile(reports[idx])
    p()
...
<head>
<meta charset="UTF-8">
<title>AntiCheat — Scan Report</title>
<style>
:root {
  --bg: #05070a;
  --panel: #0c1016;
...
<div class="container">
  ...
  <div class="section">
    <div class="section-title">Habibi Detections</div>
    {habibi_html}
  </div>
  ...
  <div class="footer">AntiCheat &middot; Stripper Algorithm &middot; Generated locally, not uploaded anywhere</div>
</div>
</body>
</html>"""
...
def generate_dashboard():
    ...
    if not habibi_html:
        habibi_html = '<div class="empty">No mod-level cheat detections.</div>'
    ...
    html = DASHBOARD_TEMPLATE.format(
        ...
        habibi_html=habibi_html, config_html=config_html,
...
def cmd_help():
    ...
    cmds = [
        ("/flash",                   "Config→Logs→Habibi→DNS→Process  (.minecraft only)"),
        ...
        ("/inspect <jar>",           "Deep Habibi scan on a single JAR"),
        ...
        ("/source",                  "List data files (cheat_strings, client_profiles, habibi)"),
    ...
    p()
    p(c(DIM,GR)("  Pipeline: Phase0·Logs  Phase1·Config  Phase2·Habibi(native)  Phase3·DNS/Process/Registry/Prefetch"))
    p(c(DIM,GR)("  ") + c(GN)("●") + c(DIM,GR)(" clean  ") + c(YL)("●") + c(DIM,GR)(" suspicious  ") +
      c(RD)("●") + c(DIM,GR)(" flagged  ·  ") + hab_widget())
    p()
...
if __name__=="__main__":
    if os.name!="nt": p("Windows only."); sys.exit(1)
    try:
